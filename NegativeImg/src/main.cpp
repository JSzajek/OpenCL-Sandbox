...
{
	/* Build program */
	program = OpenCLUtils::build_program(context, device, "shaders/negative_img.cl");
	if (!program)
		return false;
	...
	/* Create a kernel */
	kernel = clCreateKernel(program, "negative", &err);
	if (err < 0)
	{
...
int main() 
{
    ...
    if (!NegativeImage(inputImgRGBA, outputImg))
        return -1;
