...
bool BilateralFilter(const cv::Mat& input,
					 int filter_size,
					 float spatial_sigma,
					 ...
					 cv::Mat& output)
{
	...
	err |= clSetKernelArg(kernel, 1, sizeof(float), &filter_size);
	err |= clSetKernelArg(kernel, 2, sizeof(float), &spatial_sigma);
	err |= clSetKernelArg(kernel, 3, sizeof(float), &intensity_sigma);
	err |= clSetKernelArg(kernel, 4, sizeof(int), &width);
	err |= clSetKernelArg(kernel, 5, sizeof(int), &height);
    err |= clSetKernelArg(kernel, 6, sizeof(cl_mem), &output_buffer);
    if (err < 0)
    {
...
{
	/* Build program */
	program = OpenCLUtils::build_program(context, device, "shaders/bilateral_filter_img.cl");
	if (!program)
		return false;
...
int main() 
{
	...
	const float intensity_sigma = 50.0f;
    ...
    if (!BilateralFilter(inputImgRGBA, filter_size, spatial_sigma, intensity_sigma, outputImg))
        return -1;
